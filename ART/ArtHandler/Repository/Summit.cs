...
namespace ArtHandler.Repository
{
    public class Summit : Iitsmtool
    {
        ...
        public string CreateIncident(string userID, string emailID, string Usrmessage, string GSDmessage, string category, string description, string adPhysicalOfficeDelivery, ref string sys_id)
        {
            return string.Empty;
        }
        public string ResolveIncident(string ticketno, string Usrmessage, string GSDmessage, string emailID, string category, string sys_id, bool iscreateResolve)
        {
            return string.Empty;
        }
        ...
        private int CreateIncident(string userID, string status, int ticketno, string message)
        {
            ...
            try
            {
                ...
                //inc.oTicket.AssignedExecEmailID = CallerEmailID;
                 ...
                 json = JsonSerializer(inc);
                SummitAPIResponse sar = new SummitAPIResponse();
                 res = PostAPI(URL + "/" + Type, json);
            ...
            catch(Exception ex)
            {
                ...
                obj.SendMailUsingSMTP(userID, TicketNo.ToString(), URL + " - Incident creation failed  ", ex, "Request: " + json + "\n", res, "Summit Incident Creation Error");
            }
            return TicketNo;
        }
        /// <summary>
        ...
        /// <returns></returns>
        private int UpdateIncident(string status, int ticketno, string message, bool reassign, bool isFirstTime)
        {
            SummitIncident inc = new SummitIncident();
            string json = string.Empty;
            string res = string.Empty;
            try
            {
                ...
                inc.oTicket.Solution = message;
                inc.oTicket.Description = message;
                //not required-- commented while we trying to test in prod
                inc.oTicket.AssignedExecEmailID = CallerEmailID;
                inc.oTicket.InternalLog = message;
                inc.oTicket.Priority = Constants.SummitPriorityName;
                inc.oTicket.OpenCategory = Category;
                ...
                inc.oTicket.InstanceCode = InstanceCode;
                inc.oTicket.Classification = Constants.SummitClassification;
                inc.oTicket.IncidentNo = ticketno;
                inc.oTicket.Workgroup = WorkGroup;
                if (status == Constants.SummitResolvedStatus)
                {
                    inc.oTicket.ClosureCode = "Successful";
                    //inc.oTicket.Resolution_Time = DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss");
                    inc.oTicket.Resolution_Time = GetUTCNow();
                    inc.oTicket.Resolution_SLA_Reason = Constants.SummitResponse_SLA_Reason;
                }
                if (reassign)
                {
                    inc.oTicket.Workgroup = Constants.SummitSrvGrp;
                    inc.oTicket.Status = Constants.SummitAssignedStatus;
                    inc.oTicket.Response_Time = GetUTCNow();
                }

                //added on 1/8/2016
                //added on 31-08-2016 for response time input
                //inc.oTicket.Response_Time = DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss"); 
                if (status == Constants.SummitInProgressStatus && isFirstTime)
                {
                    inc.oTicket.Response_Time = GetUTCNow();
                }
                //inc.oTicket.Response_Time = null;
                inc.oTicket.Response_SLA_Met = false;
                inc.oTicket.Response_SLA_Reason = Constants.SummitResponse_SLA_Reason;

                 json = JsonSerializer(inc);
                SummitAPIResponse sar = new SummitAPIResponse();
                 res = PostAPI(URL + "/" + Type, json);
                //Utility.WriteLog("updated ticket result:" + res);

            }
            catch (Exception ex)
            {
                DAL_User obj = new DAL_User();
                obj.SendMailUsingSMTP(UserEmailID,TicketNo.ToString(), URL + " - Incident creation failed  ", ex, "Request: " + json + "\n", res, "Summit UpdateIncident Error");
                Log.LogException(new CustomException(null, ex.Message.ToString(), ex.StackTrace.ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name));
            }
            return ticketno;
        }
        ...
        /// <param name="status"></param>
        /// <param name="ticketno"></param>
        public string CreateIncident(string userID, string userEmailID, string message, string GSDmessage, string category, string description, ref string sys_id)
        {
            try
            {
                ...
                //inc.oTicket.Description = message;//description;
                //inc.oTicket.InternalLog = message;
                //inc.oTicket.Solution = message;
                ...
                //inc.oTicket.ClosureCode = "Successful";
                ////inc.oTicket.Resolution_Time = DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss");
                //inc.oTicket.Resolution_SLA_Reason = category;

                ////not required if it new
                //inc.oTicket.AssignedExecEmailID = UserName;
                ...
                //LoggingRepository.ITSMTraceLog(Constants.Summit.ToUpper(), json, res, sys_id, "Summit CreateIncident");
            ...
            string category, string description, ref string sys_id)
        {
            ...
            try
            {
                ...
                objCreate.ServiceName = Type;
                 ...
                 json = JsonConvert.SerializeObject(objCreate);
                SummitAPIResponse sar = new SummitAPIResponse();
                 res = PostAPI(URL, json);
        ...
        public string ResolveIncident(string ticketno, string message, string GSDmessage, string userEmailID, string category, string sys_id)
        {
            string json = string.Empty;
            string res = string.Empty;
            try
            {
                ...
                LoggingRepository.ITSMTraceLog(Constants.Summit.ToUpper(), json, res, sys_id, "Summit ResolveIncident");

            }
            catch (Exception ex)
            {
                DAL_User obj = new DAL_User();
                obj.SendMailUsingSMTP(userEmailID,TicketNo.ToString(), URL + " - Incident resolved failed  ", ex, "Request: " + json + "\n", res, "Summit Incident resolved Error");
                Log.LogException(new CustomException(null, ex.Message.ToString(), ex.StackTrace.ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name));
            }
        ...
        public string UpdateIncidentDetails(string status, bool reassign, string ticketno, string message, string userEmailID, string category, string sys_id)
        {
            ...
            try
            {
                ...
                LoggingRepository.ITSMTraceLog(Constants.Summit.ToUpper(), json, res, sys_id, "Summit UpdateIncident");
                
            }
            catch(Exception ex)
            {
                DAL_User obj = new DAL_User();
                obj.SendMailUsingSMTP(userEmailID,TicketNo.ToString(), URL + " - Incident updation failed  ", ex, "Request: " + json + "\n", res, "Summit Incident updation Error");
                Log.LogException(new CustomException(null, ex.Message.ToString(), ex.StackTrace.ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name));
            }
            return string.Empty;
        }
