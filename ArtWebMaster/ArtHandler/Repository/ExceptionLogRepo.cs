    ...
    public static class Log
    {
        public static bool WriteFile(string content)
        {
            try
            {
                ...
                if (!File.Exists(AppDomain.CurrentDomain.BaseDirectory + "Logs\\" + fileNameFormat + ".txt"))
                    File.Create(AppDomain.CurrentDomain.BaseDirectory + "Logs\\" + fileNameFormat + ".txt").Close();
                else
                {
                    File.SetAttributes(AppDomain.CurrentDomain.BaseDirectory + "Logs\\" + fileNameFormat + ".txt", FileAttributes.Normal);

                }
                // Write single line to new file.
                using (StreamWriter writer = new StreamWriter(AppDomain.CurrentDomain.BaseDirectory + "Logs\\" + fileNameFormat + ".txt", true))
                {
        ...
        public static void LogRequestorInfo(string userId, string sessionId, string hostName, string hostaddress, string useragent, string urlreferrer)
        {
            try
            {
                using (MySqlConnection con = MySqlConnector.OpenConnection())
